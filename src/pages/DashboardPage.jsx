...
const DashboardPage = () => {
  ...
  const leaderboard = useMemo(
    () => buildEmployeeLeaderboard(employees, matchResults),
  ...
  const selectedGameBookings = SLOTS.flatMap((slot) =>
    (dayBookings[slot.id] || []).filter((booking) => String(booking.game) === String(selectedGameRecord.id) || booking.game === selectedGameRecord.name)
  );
  ...
  const activeBanCount = bans.filter(
    (ban) =>
      ...
      (String(ban.game) === String(selectedGameRecord.id) || ban.game === selectedGameRecord.name || ban.game === 'All Games')
  ).length;
  ...
  return (
    <div style={{ display: 'grid', gap: '18px' }}>
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '18px', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
            {GAMES.map((game) => {
              const active = String(selectedGame) === String(game.id);
              return (
                <button
                  key={game.id}
                  type="button"
      ...
      <section style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.8fr', gap: '16px', alignItems: 'start' }}>
        <div className="clay-card" style={{ padding: '20px', borderRadius: '28px', background: 'rgba(255,255,255,0.96)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '14px' }}>
            <div>
              ...
              <h2 style={{ margin: '6px 0 0', fontSize: '1.05rem', color: '#1e1e2f' }}>Highest scoring players</h2>
            </div>
        ...
        <div className="clay-card" style={{ padding: '20px', borderRadius: '28px', background: 'rgba(255,255,255,0.96)' }}>
          ...
          <div style={{ display: 'grid', gap: '10px' }}>
            {SLOTS.map((slot) => {
              const players = (dayBookings[slot.id] || []).filter((booking) => String(booking.game) === String(selectedGameRecord.id) || booking.game === selectedGameRecord.name);
              return (
                <div key={slot.id} style={{ padding: '10px 12px', borderRadius: '16px', background: 'rgba(26,60,110,0.04)' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', gap: '10px', alignItems: 'center' }}>
                    ...
                    <div style={{ fontSize: '0.78rem', fontWeight: 700, color: players.length ? '#1a3c6e' : '#8888aa' }}>
                      {players.length}
                    </div>
                  </div>
                  <div style={{ marginTop: '8px', display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                    {players.length > 0 ? players.map((player) => (
                      ...
                      <span style={{ fontSize: '0.68rem', color: '#8888aa' }}>No bookings yet</span>
                    )}
                  </div>
                </div>
              );
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px', flexWrap: 'wrap', alignItems: 'center', marginBottom: '14px' }}>
          <div>
            ...
            <h2 style={{ margin: '6px 0 0', fontSize: '1.05rem', color: '#1e1e2f' }}>
              All employees with points and ranking
            </h2>
          </div>
          <div style={{ fontSize: '0.72rem', color: '#666' }}>
